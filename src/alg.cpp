...
PMTree::PMTree(const std::vector<char>& src) : base(src) {
  ...
  for (char ch : sorted_src) {
    ...
    if (it != leftover.end()) leftover.erase(it);
    Node* child = BuildSubtree(leftover);
    child->sym = ch;
...
PMTree::Node* PMTree::BuildSubtree(const std::vector<char>& rest) {
  ...
  for (char ch : sorted_rest) {
    std::vector<char> next = sorted_rest;
    auto pos = std::find(next.begin(), next.end(), ch);
    if (pos != next.end()) next.erase(pos);
    Node* child = BuildSubtree(next);
    child->sym = ch;
...
std::vector<char> getPerm2(PMTree& obj, int pos) {
  ...
  for (int step = 0; step < total; ++step) {
    ...
    if (idx >= static_cast<int>(current->links.size())) return {};
